      ...
      <div className="relative mb-4 sm:mb-6 lg:mb-8">
        <div className="overflow-x-auto pb-2 -mx-2 px-2">
          <div className="min-w-max relative">
            ...
            />
            <div className="relative flex items-center px-1 sm:px-2 lg:px-4 justify-start gap-4 sm:gap-6 lg:gap-8">
              {TRACKING_STAGES.map((stage, index) => (
                <div
                  key={stage.id}
                  className="flex flex-col items-center group flex-shrink-0"
                >
                  <div
                    className={`relative w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-full border-2 sm:border-3 flex items-center justify-center text-xs sm:text-sm font-bold transition-all duration-500 transform hover:scale-105 ${
                      index <= currentStageIndex
                        ? "bg-gradient-to-br from-blue-500 to-blue-600 border-blue-500 text-white shadow-lg shadow-blue-500/25"
                        : index === currentStageIndex + 1
                        ? "bg-white border-blue-300 text-blue-400 shadow-md ring-1 sm:ring-2 ring-blue-200"
                  ...
                  <div className="mt-1.5 sm:mt-2 lg:mt-3 text-xs sm:text-xs lg:text-sm text-center w-16 sm:w-20 lg:w-24 leading-tight transition-all duration-300 break-words hyphens-auto text-blue-700 font-semibold">
                    {stage.name}
                  </div>
                </div>
              ))}
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden mb-6">
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            {trackingData.map((entry, index) => {
              const stageIndex = getCurrentStageIndex(entry.stage);
              return (
                <HistoryEntry
                  key={index}
                  dateTime={formatDateTime(entry.createdAt)}
                  location={entry.currentLocation}
                  status={TRACKING_STAGES[stageIndex]?.name || "Unknown"}
                  updatedBy={entry.updatedBy}
                  remarks={entry.comment}
                />
              );
